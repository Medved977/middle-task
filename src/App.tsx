...
function App() {
    ...
    const getDataWithFilters = (data: Users, filters: IFilters): Users => {
      ...
      return data.filter((row) => {
        ...
        for (let key in filters) {
          if (!filters[key as keyof IFilters]) continue;

          const filterType = columns.find((col) => col.key === key)?.filterType;

          if (filterType === "includes") {
            const filterValue = (
              filters[key as keyof IFilters] as string
            ).toLowerCase();
            const value = row[key as keyof IUser];

            if (typeof value === "string") {
              const invalidIncludes = !value
                .toLowerCase()
          ...
          if (filterType === "equality") {
            const filterValue = filters[key as keyof IFilters];
            const invalidEquality = row[key as keyof IUser] !== filterValue;
          ...
          if (filterType === "range") {
            ...
            if (start && end) {
              const inRange =
                +start <= +row[key as keyof IUser] &&
                +row[key as keyof IUser] <= +end;
  ...
  const onRangeChanged = (
    ...
    setFilters({
      ...filters,
      [key]: {
        ...(filters[key as keyof IFilters] as Record<string, string>),
